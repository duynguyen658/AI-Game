...
The asset row is locked while allocating the next attempt number, so retries are unique under concurrency. Provider and storage calls run outside transactions. Success then locks the live job lease and atomically changes the attempt to `COMPLETED`, the asset to `READY_FOR_REVIEW`, the task to `COMPLETED`, and inserts the ready outbox event. A stale worker cannot write success or an outbox event.

The outer execution guard also covers safety, storage, outbox, and final commit failures. Cancellation cleanup is shielded. Terminal job reconciliation changes any remaining active attempt to `FAILED` or `CANCELLED` using expected-state guards. No media attempt remains `STARTED` after a terminal job state.
